...
#some words such as the 'arkansas' river, appears in the state place table as 'arkansas river', remove this kind of word out of state_place
        ...
        #load key word
        self.key_w = []
        self.key_w = self._read_list(GEO_KEY1,self.key_w, True)

        self.human_info = {}
        self.human_info = self._read_list_special(GEO_KEY2,'human_info',True)

        self.special_w = {}
        self.special_w = self._read_list_special(GEO_KEY3,'special',True)
    ...
    def _import_data(self, db_name, col_name, file = GEO_TABLE):
