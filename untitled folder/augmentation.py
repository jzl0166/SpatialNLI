...
class Augmentation:
    def __init__(self):
        ...
        with open('train_org.qu', 'r') as f1,\
            open('train_ground_truth.txt','r') as f2:
            ...
            for i,line2 in enumerate(l2): #input ground truth and substitute the constant phrase to get a new sentence
                line2 = line2.strip('\n').split(' ')
                line2 = [item for item in filter(lambda x:x != '', line2)]
                
                #line2_org = ' '.join(line2)
                self.res_ans.append(' '.join(line2))
                
                line2_, word = self.substitute(line2) #find out the constant word
...
#=====> 'what is the minimum mountain in florida'
            ...
            with open('qu_absw.txt', 'w') as f:
                f.write('\n'.join(self.res_q))
            with open('ans_absw.txt', 'w') as f:
                f.write('\n'.join(self.res_ans))
...
#'A' will be replaced by 'C', but then 'C' will continue to be replaced, so use @ to mark that as being replaced
    ...
    #some words such as the 'arkansas' river, appears in the state place table as 'arkansas river', remove this kind of word out of state_place
        ...
        self.specific = self.city + self.state_name + self.state_abbre + self.state_place + self.country + self.lake + self.mountain + self.river
